...
Querying a state will return a data frame with rows each with a county and its
associated FMR. A (state fipscode + county fipscode + county subdivision code) 10 digit code will return a 1 row data frame for that county. A CBSA code will also return a 1 row data frame for that area. 

This is different from the hud_il() for state queries, as FMR is not defined 
at a state resolution.
