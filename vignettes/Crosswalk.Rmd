...
1) [zip-tract][Converting zipcode to census tract]
2) [zip-county][Converting zipcode to county fip]
3) [zip-cbsa (Core Base Statistical Areas)][Converting zipcode to core base statistical area (cbsa)]
4) [zip-cbsadiv (Available 4th Quarter 2017 onwards)][Converting zipcode to core based statistical area (cbsadiv)]
5) [zip-cd (Congressional District)][Converting zipcode to congressional district (cd)]
6) [tract-zip][Converting census tract to zipcode]
7) [county-zip][Converting county fip into zipcode]
8) [cbsa-zip][Converting core based statistical areas (cbsa) to zipcode]
9) [cbsadiv-zip (Available 4th Quarter 2017 onwards)][Converting core based statistical areas division (cbsadiv) to zipcode]
10) [cd-zip][Converting congressional district (cd) to zipcode]
11) [zip-countysub (Available 2nd Quarter 2018 onwards)][Converting zipcode to county subdivision (countysub)]
12) [countysub-zip (Available 2nd Quarter 2018 onwards)][Converting county subdivision (countysub) to zipcode]
...
the measurements are COMPLETELY inverse -- for reasons stated within the papers
above.
...
### Converting zipcode to census tract
...
```
### Converting zipcode to county fip
```{r, eval = if(Sys.getenv("HUD_KEY") == "") FALSE else TRUE}
hud_cw_zip_county(zip = 35213, year = c('2020'), quarter = c('2'))
```
### Converting zipcode to core base statistical area (cbsa)
```{r, eval = if(Sys.getenv("HUD_KEY") == "")  FALSE else TRUE}
hud_cw_zip_cbsa(zip = 35213, year = c('2011'), quarter = c('3'))
```
### Converting zipcode to core based statistical area (cbsadiv)
```{r, eval = if(Sys.getenv("HUD_KEY") == "")  FALSE else TRUE}
hud_cw_zip_cbsadiv(zip = '22031', year = c('2019'), quarter = c('4'))
```
### Converting zipcode to congressional district (cd)
```{r, eval = if(Sys.getenv("HUD_KEY") == "")  FALSE else TRUE}
hud_cw_zip_cd(zip = '35213', year = c(2011), quarter = c(1))
```
### Converting census tract to zipcode
```{r, eval = if(Sys.getenv("HUD_KEY") == "")  FALSE else TRUE}
hud_cw_tract_zip(tract = 48201223100, year = c('2017'), quarter = c('1'))
```
### Converting county fip into zipcode 
```{r, eval = if(Sys.getenv("HUD_KEY") == "")  FALSE else TRUE}
hud_cw_county_zip(county = '22031', year = c('2010'), quarter = c('1'))
```
### Converting core based statistical areas (cbsa) to zipcode
```{r, eval = if(Sys.getenv("HUD_KEY") == "")  FALSE else TRUE}
hud_cw_cbsa_zip(cbsa = '10140', year = c('2017'), quarter = c('2'))
```
### Converting core based statistical areas division (cbsadiv) to zipcode
```{r, eval = if(Sys.getenv("HUD_KEY") == "")  FALSE else TRUE}
hud_cw_cbsadiv_zip(cbsadiv = 10380, year = c('2017'), quarter = c('4'))
```
### Converting congressional district (cd) to zipcode
```{r, eval = if(Sys.getenv("HUD_KEY") == "")  FALSE else TRUE}
hud_cw_cd_zip(cd = '2202', year = c('2010'), quarter = c('4'))
```
### Converting zipcode to county subdivision (countysub)
```{r, eval = if(Sys.getenv("HUD_KEY") == "") FALSE else TRUE}
hud_cw_zip_countysub(zip = '35213', year = c('2019'), quarter = c('2'))
```
### Converting county subdivision (countysub) to zipcode 
```{r, eval = if(Sys.getenv("HUD_KEY") == "")  FALSE else TRUE}
...
<table>
	<tr>
 	...
 	<tr>
  		<td>zip/county/fipstract/cbsa/cbsadiv/cd/countysub</td>
   		<td>The geoid to query for</td>
 	</tr>
	<tr>
  		<td>res_ratio</td>
      ...
      Zip, Tract, County, or CBSA. (for type 6-10)</td>
 	</tr>
	<tr>
  		<td>bus_ratio</td>
	...
	<tr>
      ...
      other addresses in the entire Tract, County, or CBSA. (for type 6-10)</td>
 	</tr>
 	<tr>
  		<td>tot_ratio</td>
...
|       https://www.census.gov/newsroom/blogs/random-samplings/2014/07/understanding-geographic-relationships-counti |       es-places-tracts-and-more.html
|
