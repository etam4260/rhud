...
This function returns a single row dataframe for each query.
...
**TYPE** Must be a number between 1 and 5 depending on the specificity of the
region.
...
**ENTITYID** If type is 3-5, you must specify either the county, mcd, or place,
respectively.
    
  - This will correspond to the county fips code of 039
  - Use hud_counties("VA") to find fips codes for the state of Virginia
  entityId = 39
  
  - Use hud_minor_civil_divisions("VA") to find MCD codes for the state of Virginia
  entityId = 93927
  
  Use hud_cities("VA") to find city codes for the state of Virginia
  entityId = 48996
