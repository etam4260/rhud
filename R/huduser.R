...
#' @param year Gets the year that this data was recorded. Default is the
#' latest year.
...
hud_fmr <- function(query, year = 2021, key = Sys.getenv("HUD_KEY")) {
  URL <- NULL
  call <- NULL
  cont <- NULL
  ...
  URL <- paste("https://www.huduser.gov/hudapi/public/fmr/", if(!numbers_only(query))
    "statedata/" else "data/", query, "?year=", year, sep="") #build URL
  ...
  if (length(cont$data) == 0){
    ...
    if(!numbers_only(query)) {
      return(as.data.frame(do.call(rbind, cont$data$counties)))
    } else {
...
hud_il <- function(query, year = 2021, key = Sys.getenv("HUD_KEY")) {
  URL <- NULL
  call <- NULL
  cont <- NULL
