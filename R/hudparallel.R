...
api_call <- function(listed) {
  call<-try(GET(as.character(listed['url']), add_headers(Authorization=paste("Bearer ", listed['key'])), user_agent("https://github.com/etam4260/hudr"), timeout(30)),silent = TRUE) #try to make call
