...
chas_input_check_cleansing <- function(query, year, key) {
  ...
  for(i in 1:length(year)) {
    if(!any(year %in% c("2014-2018","2013-2017","2012-2016","2011-2015","2010-2014",
                        "2009-2013","2008-2012","2007-2011","2006-2010"))) stop("One of the years does not fall in the correct range of values.")
  }
...
fmr_il_input_check_cleansing <- function(query, year, key) {
  ...
  if(nrow(pkg.env$state[pkg.env$state$state_name %in% as.character(query),]) != 0) {
    query <- pkg.env$state[pkg.env$state$state_name %in% as.character(query),][2]
  }
  if(nrow(pkg.env$state[pkg.env$state$state_code %in% as.character(query),]) != 0) {
    query <- pkg.env$state[pkg.env$state$state_code %in% as.character(query),][2]
  }
  if(nrow(pkg.env$state[as.character(pkg.env$state$state_num) %in% as.character(query),]) != 0) {
    query <- pkg.env$state[pkg.env$state$state_num %in% as.character(query),][2]
  }
