...
# These methods provide access to non api based datasets. Currently they
# download the files directly from the internet, but should be made more
# generic. Maybe scraping the site for pages. Doing a depth first search of
# all html pages under HUD and scraping for datasets and putting them together.
...
#' @export
hud_lihtc <- function(file = 1) {
  if(file == 1) {
    # County level
    return(fread("https://lihtc.huduser.gov/lihtcpub.csv", header = TRUE,
...
#' @name hud_nsp
#' @title hud_nsp
#' @description Returns a data frame pertaining to the Neighborhood
#' Stabilization Program provided by HUD User or HUD Exchange
#' @param file The specific file needed.
#' @returns A data frame built from file.
#' @export
hud_nsp <- function(file = 1) {
  if(file == 1) {
...
#' @name hud_adjf
#' @title hud_adjf
#' @description Returns a data frame from HUD USER pertaining to the
#' Annual Adjustment Factors dataset.
#' @param year The specific year for the Annual Adjustment Factors to query for.
#' @param sheet The specific sheet in the file.
#' @returns A data frame built from the file.
#' @export
hud_adjf <- function(year = 2021, sheet = 1) {
  return(read.xlsx(paste("https://www.huduser.gov/portal/datasets/aaf/",
                         as.character(year), "ScheduleC.xlsx", sep = ""),
                   sheet = sheet))
}


# Difficult Development Areas (DDA) are areas with high land, construction
# and utility costs relative to the area median income and are based on
# Fair Market Rents, income limits, the 2010 census counts, and 5-year American
# Community Survey (ACS) data. Maps of Qualified Census Tracts and Difficult
# Development Areas are available at: huduser.gov/sadda/sadda_qct.html.
...
#' @returns A data frame built from the file.
#' @export
hud_dda <- function(year = 2022 , sheet = 1) {
  return(read.xlsx(paste("https://www.huduser.gov/portal/datasets/qct/",
                         as.character(year), "-DDAs-Data-Used-to-Designate.xlsx",
                         sep = ""), sheet = sheet))
}
...
# The Homeowner Assistance Fund (HAF) Income Limits are used for determining
# eligibility for HAF funds. Supports 2022 data. Should be 1 sheet.

#' @name hud_haf
#' @title hud_haf
#' @description Returns a data frame from HUD USER pertaining to the Homeowner
#' Assistance Fund Income Limits
#' @param year The specific year for the Homeowner Assistance Fund Income Limits
#' to query for.
#' @returns A data frame built from the file.
#' @export
hud_haf <- function(year = 2022) {
  return(read.xlsx("https://www.huduser.gov/portal/sites/default/files/datasets/haf-il/il21_all100_150_HAF.xlsx"))
}


# Rent estimates at the 50th percentile (or median) are calculated for all Fair
# Market Rent areas. THESE ARE NOT FAIR MARKET RENTS. Under certain conditions,
# as set forth in the Interim Rule (Federal Register Vol. 65, No. 191,
# Monday October 2, 2000, pages 58870-58875), these 50th percentile rents
# can be used to set success rate payment standards.
# Can specify the resolution of the 50th percentile by either
# "c" for county or "a" for area.
...
#' @returns A data frame built from the file.
#' @export
hud_50p <- function(year = 2022, res = "c") {
  if(res == "c") {
    return(read.xlsx(paste("https://www.huduser.gov/portal/datasets/50thper/FY",
                           2022, "_50_County.xlsx", sep = "")))
  } else {
    return(read.xlsx(paste("https://www.huduser.gov/portal/datasets/50thper/FY",
                           2022, "_50_FMRArea.xlsx", sep = "")))
  }
}

# The Housing Affordability Data System (HADS) is a set of files derived from
# the 1985 and later national American Housing Survey (AHS) and the 2002 and
# later Metro AHS. This system categorizes housing units by affordability
# and households by income, with respect to the Adjusted Median Income,
# Fair Market Rent (FMR), and poverty income. It also includes
# housing cost burden for owner and renter households. These files have been
# the basis for the worst case needs tables since 2001. The data files are
# available for public use, since they were derived from AHS public use files
# and the published income limits and FMRs. We are providing these files give
# the community of housing analysts the opportunity to use a consistent set of
# affordability measures.

# Years are recorded every two years (2013,2011,2009).
# No data past 2013 available.

#' @name hud_hads
#' @title hud_hads
#' @description Returns a data frame from HUD USER pertaining to the
#' Housing Affordability Data System.
#' @param year The specific year for the Housing Affordability Data System to
#' query for.
#' Years are recorded every two years (2013,2011,2009).
#' No data past 2013 available.
#' @returns A data frame built from the file.
#' @export
hud_hads <- function(year = 2013) {
...
# of all of the characteristics, cautions, and explanations about some of the
# results. Each report has two sections. The first section, labeled "Losses,"
# shows the losses to the housing stock in the two-year period using the first
# year as the base year. In other words, the first section shows the disposition
# of all the units that were present at the beginning of the period. The second
# section, labeled "Gains," shows the gains in the housing stock during the two-year
...
# Provides data from 2001 to 2017. They are in multiples of two. So 2017-2015,
# 2011-2013, 2009-2011, 2009 -2007, 2003-2005, 2001-2003
...
#' @param year The specific year for the Components of Inventory Change
#' (CINCH) report to query for. Years must be a character type specifying
#' a year range:
#' 1) 2017-2015,
#' 2) 2011-2013,
#' 3) 2009-2011,
...
#' @returns A data frame built from the file.
#' @export
hud_cinch <- function(year = '2017-2015') {
...
#' @param year The specific year for the Point-In-Time (PIT) count to query for.
#' Supports years 2020 to 2007.
...
#' @returns A data frame built from the file.
#' @export
hud_pit <- function(year = 2020, res = "s", pop = "s") {
  if(res == "s") {
    if(pop == "s") {
      years = seq(2020, 2007)
...
#' @param year The specific year for the Housing Inventory Count (HIC) count
#' to query for. Supports years 2020 to 2007.
#' @param res The resolution of the dataset. Either Coc or State level data.
#' @returns A data frame built from the file.
#' @export
hud_hic <- function(year = 2020, res = "s") {
  years = seq(2020, 2007)
  if(res == "s") {
...
# housing and existing housing based on 95 percent of the median purchase price
# for the area. Supports years 2021-2014
# Both HTF and HOME are in same dataset.

#' @name hud_hvl
#' @title hud_hvl
#' @description Returns a data frame from HUD USER pertaining to the
#' HOME Investment Partnerships Program (HOME) and Housing Trust Fund (HTF).
#' @param year The specific year for the HOME Investment Partnerships
#' Program (HOME) and Housing Trust Fund (HTF) count to query for.
#' Supports years 2021 to 2014.
#' @returns A data frame built from the file.
#' @export
hud_hvl <- function(year = 2021) {
  return(read.xlsx("https://www.huduser.gov/portal/datasets/home-datasets/files/FY-",
                   as.character(year), "-HOME-HTF-Homeownership-Sales-Limits.xlsx"))
}
...
# tax exempt housing bonds issued to provide qualified residential rental
# development under section 142 of the Code.
...
#' @param year The year to query for in Multifamily Tax Subsidy Projects (MTSP)
#' Income Limits. Supports years 2021 to 2014.
#' @param avg The averaged or non averaged dataset.
#' @returns A data frame built from the file.
#' @export
hud_msil <- function(year = 2021, avg = FALSE) {
  year = substr(as.character(year), nchar(year) - 1, nchar(year))
  if(avg) {
    return(read.xlsx(paste("https://www.huduser.gov/portal/datasets/mtsp/mtsp",
                           year, "/MTSP-Data.xlsx")))
  } else {
    return(read.xlsx(paste("https://www.huduser.gov/portal/datasets/mtsp/mtsp",
                           year, "/MTSP_IncAvg_Data.xlsx")))
  }
}
...
#' @name hud_muaf
#' @title hud_muaf
#' @description Returns a data frame from HUD USER pertaining to the Utility
#' Allowance Factors (UAFs) for projects.
#' @param year The specific year for the Utility Allowance Factors (UAFs) for
#' projects. Supports years 2022 to 2015.
#' @returns A data frame built from the file.
#' @export
hud_muaf <- function(year = 2022) {
  year = substr(as.character(year), nchar(year) - 1, nchar(year))
  return(read.xlsx("https://www.huduser.gov/portal/datasets/UtilAllow_FY",
                   as.character(year), ".xlsx"))
}
...
#' @name hud_fif
#' @title hud_fif
#' @description Returns a data frame from HUD USER pertaining to the Renewal
#' Funding Inflation Factors.
#' @param year The specific year for the Renewal Funding Inflation Factors.
#' Supports years 2021 to 2017.
#' @returns A data frame built from the file.
#' @export
hud_fif <- function(year = 2021) {
  return(read.xlsx("https://www.huduser.gov/portal/datasets/rfif/FY",
                   as.character(year), "/RFIF_", as.character(year), ".xlsx"))
}
...
#' @name hud_sfmr
#' @title hud_sfmr
#' @description Returns a data frame from HUD USER pertaining to the Small
#' Area Fair Market Rents (SAFMRs).
#' @param year The specific year for the Small Area Fair Market Rents.
#' Supports years 2022 to 2016.
#' @returns A data frame built from the file.
#' @export
hud_sfmr <- function(year = 2022) {
  return(read.xlsx("https://www.huduser.gov/portal/datasets/fmr/fmr",
                   as.character(year), "/fy", as.character(year), "_safmrs.xlsx"))
}
...
#' @name hud_ash
#' @title hud_ash
#' @description Returns a data frame from HUD USER pertaining to the Picture of
#' Subsidized Households.
#' @param year The specific year for the Picture of Subsidized Households.
#' Supports years 2021 to 2009.
#' @param res The GEOID or place resolution(2012-2021 are based
...
#' @returns A data frame built from the file.
#' @export
hud_ash <- function(year = 2021, res = 1) {
  res <- switch(res,
         "US",
         "STATE",
         "CBSA",
  ...
  return(read.xlsx("https://www.huduser.gov/portal/datasets/pictures/files/",
                   res, "_", as.character(year), ".xlsx"))
}
