...
test_that("Income Limits Different Years", {
  y1 <- hud_fmr("VA", year=c(2021))
  expect_true(nrow(y1) >= 1)
  y2 <- hud_fmr("WY", year=c(2021, '2019'))
  expect_true(nrow(y2) >= 1)
  y3 <- hud_fmr("MD", year=c('2021', '2018'))
  expect_true(nrow(y3) >= 1)
  y4 <- hud_fmr("CA", year=c(2015, 2017))
  expect_true(nrow(y4) >= 1)
  y5 <- hud_fmr("NY", year=c('2021', '2019', '2021'))
